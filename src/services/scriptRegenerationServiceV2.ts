...
export class ScriptRegenerationServiceV2 {
  ...
  private handleScriptGenerationCompleted(conversationId: string, scriptId: string): void {
    console.debug('Script generation completed', {
      conversationId,
      scriptId
    })

    // Auto-regeneration will be triggered by ConversationProvider after sections are marked as completed
    console.debug('Auto-regeneration will be triggered after sections are completed')
  }

  private handleSectionRegenerationCompleted(conversationId: string, scriptId: string, sectionId: string): void {
    console.debug('Section regeneration completed, checking for additional auto-regeneration', {
      conversationId,
      scriptId,
      sectionId
    })
  ...
  requestRegenerations(
    ...
    console.debug('Requesting next auto-regeneration (sequential)', {
    ...
    console.debug('Queueing next section for regeneration', {
  ...
  requestManualRegeneration(
    ...
    console.debug('Requesting manual regeneration', {
      sectionId,
      sectionTitle
    })
  ...
  handleAutoRegenerationCheck(
    ...
    console.debug('Section analysis results:', {
    ...
    if (sectionsToRegenerate.length > 0) {
      console.debug('Auto-regeneration analysis complete', {
        totalSections: conversation.sections.length,
      ...
      console.debug('No sections need auto-regeneration', {
        totalSections: conversation.sections.length
      })
    }
  }
  ...
  updateRules(newRules: Partial<RegenerationState['rules']>): void {
    ...
    console.debug('Rules updated', newRules)
  }
